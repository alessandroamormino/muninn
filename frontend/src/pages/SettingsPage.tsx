...
import { useMetricsBuffer } from './settings/useMetricsBuffer'
...
function ContainerCard({ name, metric, getBuffer }: {
  name: string
...
export default function SettingsPage() {
  ...
  const { push, get } = useMetricsBuffer()

  // Push every successful poll's totals + per-container CPU/RAM into the ring buffer.
  useEffect(() => {
    if (!data) return
    push('cpu:totals', data.totals.cpu_pct)
    push('mem:totals', data.totals.mem_used)
    for (const c of data.containers) {
      push(`cpu:${c.name}`, c.cpu_pct)
      push(`mem:${c.name}`, c.mem_used)
    }
  }, [data, push])
          ...
          <section>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {data.containers.map((c) => (
                <ContainerCard key={c.name} name={c.name} metric={c} getBuffer={get} />
              ))}
            </div>
          </section>
        </div>
