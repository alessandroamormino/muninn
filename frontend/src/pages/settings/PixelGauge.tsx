...
//
// SIZING: the grid divides its container in BOTH axes (template-columns/rows are
// `1fr`), so cells STRETCH to fill whatever box `className` gives it. The gauge is
// meant to occupy all the width left over beside the number — the caller sets
// `flex-1` for width and a bounded `h-*` so it fills the gap without ever
// overflowing the card. Cells may end up rectangular on very wide cards; that is an
// accepted trade-off for "fill the available width".
...
interface Props {
  ...
  cols?: number
  rows?: number
  /** Sizing for the gauge box — caller MUST give it a width (e.g. flex-1) and a bounded height. */
  className?: string
}

export function PixelGauge({ fraction, cols = 28, rows = 5, className }: Props) {
